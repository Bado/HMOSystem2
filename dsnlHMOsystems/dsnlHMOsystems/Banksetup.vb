...
Public Class Banksetup
    ...
    Private Sub ToolStripButton4_Click(sender As Object, e As EventArgs) Handles ToolStripButton4.Click
        Me.Close()
    End Sub
    Private Sub Banksetup_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Sub loagriddata()
        ...
        For i2 = i2 To bankgrid.RowCount - 1
            bankgrid.Rows.Clear()
        Next
        Dim gg2 As String = "SELECT BANKCODE,NAMES,sortcode,BRANCH,CMANAGER1,address FROM BANKSETUP where RTrim(bankcode) = '" & RTrim(BANKCODE.Text) & "'"
        Try
            Using ccnn As New SqlConnection(My.Settings.cnnstring)
                ...
                If r.HasRows() Then
                    While r.Read
                        ...
                        bankgrid.Rows(row).Cells(0).Value = r(2)
                        bankgrid.Rows(row).Cells(1).Value = r(3)
                        bankgrid.Rows(row).Cells(2).Value = r(4)
                        bankgrid.Rows(row).Cells(3).Value = r(5)
                    End While
                Else
                    Names.Focus()
                End If
    ...
    Sub delbankfirst()
        ...
        Dim cSQL5 As String = "DELETE FROM BANKSETUP where RTrim(BANKCODE) = '" & RTrim(BANKCODE.Text) & "'"
        Try
            Using CCNN As New SqlConnection(My.Settings.cnnstring)
                Dim Cd As SqlCommand = CCNN.CreateCommand
                Cd.CommandType = CommandType.Text
    ...
    Public Sub InsertRecordperiod()  'Subroutine to insert a record into the database  
        delbankfirst()
        cSQL = "INSERT INTO BANKSETUP(BANKCODE,NAMES) VALUES('" & Me.BANKCODE.Text & "','" & Me.Names.Text & "')"
        Try
            Using CCNN As New SqlConnection(My.Settings.cnnstring)
                Dim Cd As SqlCommand = CCNN.CreateCommand
                Cd.CommandText = cSQL
        ...
        '                    Cd3.CommandText = "INSERT INTO BANKSETUP(BANKCODE,NAMES,sortcode,BRANCH,CMANAGER1,address)" & _
        '                        "VALUES('" & BANKCODE.Text & "','" & Names.Text & "','" & bankgrid.Rows(i).Cells(0).Value & "','" & bankgrid.Rows(i).Cells(1).Value & "','" & bankgrid.Rows(i).Cells(2).Value & "','" & bankgrid.Rows(i).Cells(3).Value & "')"
        '                    CCNN.Open()
        '                    Cd3.ExecuteNonQuery()
        '                    CCNN.Close()
        '                End If
    ...
    Private Sub BANKCODE_TextChanged(sender As Object, e As EventArgs)
    ...
    Private Sub BANKCODE_TextChanged_1(sender As Object, e As EventArgs) Handles BANKCODE.TextChanged
