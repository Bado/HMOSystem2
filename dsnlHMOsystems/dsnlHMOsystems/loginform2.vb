...
Public Class LoginForm2
    ...
    Sub loadpix()
        Try
            Using cn As New SqlConnection(My.Settings.cnnstring)
                cn.Open()
                Dim sql As String = "SELECT staffid,ppicture FROM stafftab WHERE" _
                ...
                If dr.HasRows Then
                    ...
                    MDIParent1.staffpix.Image = Image.FromStream(ms)
                End If
                cn.Close()
            End Using
        ...
        '    Using CCNN As New SqlConnection(My.Settings.cnnstring)
        '        Dim Cd As SqlCommand = CCNN.CreateCommand
        '        Cd.CommandText = "SELECT staffid,ppicture FROM stafftab WHERE" _
        '        & " staffid = '" & RTrim(userid.Text) & "'"
        '        Cd.CommandType = CommandType.Text
        '        CCNN.Open()
        '        Dim r As SqlDataReader = Cd.ExecuteReader
    ...
    Private Sub LoginForm2_Disposed(sender As Object, e As EventArgs) Handles Me.Disposed
        ...
        Try
            Using cn As New SqlConnection(My.Settings.cnnstring)
                ...
                If dr.HasRows Then
                    ...
                    Dim data2 As Byte() = DirectCast(dr("picpath"), Byte())
                    Dim ms2 As New MemoryStream(data2)
                    MDIParent1.COYPICS.Image = Image.FromStream(ms2)
                End If
                cn.Close()
            End Using
        ...
        '            ' ''Dim ms As New MemoryStream(data)
        '            ' ''MDIParent1.COYPICS.Image = Image.FromStream(ms)
        '            ' ''Dim data2 As Byte() = DirectCast(r("backgroundimg"), Byte())
        '            ' ''Dim ms2 As New MemoryStream(data2)
        '            ' ''MDIParent1.BackgroundImage = Image.FromStream(ms2)
