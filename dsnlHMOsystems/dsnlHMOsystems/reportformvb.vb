...
Public Class reportformvb
    ...
    Sub Printenrollehcpall()
        My.Settings.reportsqlstr = "Printenrollehcpall"
        Dim viewreport2 As New viewreport
        viewreport2.MdiParent = MDIParent1
        rptrangecode = codeidall.Text
        viewreport2.Text = " Print Enrolles By HCP"
    ...
    Private Sub Button4_Click(sender As Object, e As EventArgs) Handles Button4.Click
        ...
        If range.Checked = True Then
            If RTrim(rptid) = "137" Or RTrim(rptid) = "138" Or RTrim(rptid) = "143" Then
                Try
                    gsql = ""
                    DESCC = ""
        ...
        End If
        If range.Checked = True Then
            If RTrim(rptid) = "128" Or RTrim(rptid) = "146" Then
                Try
                    gsql = ""
                    DESCC = ""
                    OPTRR = publicvarmodule.OPT
        ...
        End If
        If range.Checked = True Then
            If RTrim(rptid) = "106" Or RTrim(rptid) = "145" Then
                Try
                    gsql = ""
                    DESCC = ""
                    OPTRR = publicvarmodule.OPT
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        ...
        If RTrim(rptid) = "143" Then
            ...
            End If
        End If
        If RTrim(rptid) = "145" Then
            If range.Checked = True Then
                printenrolleedepentsector()
            End If
        End If
        If RTrim(rptid) = "146" Then
            If range.Checked = True Then
    ...
    Sub printenrolleedepenthcp()
        UPDATEenrodepend()
        rptrangecode = ""
        My.Settings.reportsqlstr = "printenrolleedepenthcp"
        rptrangecode = codeidall.Text
        Dim viewreport2 As New viewreport
        viewreport2.MdiParent = MDIParent1
    ...
    Sub printenrolleedepentsector()
        UPDATEenrodepend()
        rptrangecode = ""
        My.Settings.reportsqlstr = "printenrolleedepentsector"
        rptrangecode = codeidall.Text
        Dim viewreport2 As New viewreport
        viewreport2.MdiParent = MDIParent1
        viewreport2.Text = "Print Enrollee/dependant Report"
    ...
    Private Sub Button8_Click(sender As Object, e As EventArgs) Handles Button8.Click
        Me.Close()
    End Sub
    Public Sub deleenrodepend()
        Dim cSQL As String = "DELETE FROM Familyinfoprint"
        Try
            Using CCNN As New SqlConnection(My.Settings.cnnstring)
                Dim Cd As SqlCommand = CCNN.CreateCommand
                Cd.CommandType = CommandType.Text
    ...
    Public Sub UPDATEenrodepend()  'Subroutine to insert a record into the database  
        deleenrodepend()
        Try
            Dim sumSQL As String = "INSERT INTO Familyinfoprint(ftype, names, alterHCP, dofb, age, sex, Enrolleeid)" & _
                                                     "SELECT  ftype, names, alterHCP, dofb, age, sex, Enrolleeid FROM Familyinfo"
            ...
            UPDATEenroprincipal()
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Occurs in UPDATE Familyinfoprint Insert")
        End Try

    End Sub

    Public Sub UPDATEenroprincipal()
        Try
            Dim sumSQL As String = "INSERT INTO Familyinfoprint(ftype, names, alterHCP, dofb, age, sex, Enrolleeid)" & _
                                                     "SELECT  'Principal' as ftype, rtrim(surname) + ' ' + rtrim(firstname) + ' ' + rtrim(othernames) as names,'' as  alterHCP, dofb, age, sex, Enrolleeid FROM enrolleetab where active = '" & False & "'"
